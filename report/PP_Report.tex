...
\section{Performance}
\label{perf}
\IEEEPARstart{I}{n} this section we describe the current state of the project,
the performance analysis that has been done, and the optimizations that are
planned.

\subsection{Impact of each method}

\subsection{Optimizations}
The rendering of a single frame contains three time-consuming operations (see
Figure~\ref{fig:prof}):
\begin{itemize}
\item Device memory allocation (\texttt{cudaMalloc()}) for the scene data (objects, lights, etc.),
\item Main kernel (\texttt{rt\_eachpx()}),
\item Data transfer from device to host (\texttt{cudaMemcpy()}).
\end{itemize}
In non-interactive mode, the bottleneck is \texttt{cudaMalloc()}.  This is explained by the fact that the first call to \texttt{cudaMalloc()} incurs an inevitable cost that is orders of magnitude higher than the computation time of the main kernel or the data transfer.  When rendering a single frame, this is very significant and this allocation represents between 80\% and 90\% of the total runtime of our application.  But in interactive mode this cost is only paid once, and the subsequent allocations take less than a millisecond each.

The kernel itself takes between 50ms and 100ms with a 1Mpx image (1000x1000). The \texttt{cudaMemcpy()} is negligible for now (below 1ms), which means that we can consistently display between 10 and 20 frames per second in interactive mode.
...
\begin{itemize}
\item Warp divergence: the main kernel contains many branching statements that probably slow down the application because of warp divergence.  We want to measure the impact of this divergence in terms of performance and rewrite the kernel accordingly.
\item Try more complex scenes and analyze the performance of the kernel itself.
\item Play with different block sizes, dimensions, and number of threads per blocks.
\end{itemize}
...
\label{diff}
\IEEEPARstart{W}{e} faced a few technical issues when translating our first C++ implementation to CUDA.
\subsection{Derived classes}
Instances of derived class (such as \texttt{sphere}, \texttt{plane} and \texttt{triangle} which inherit from \texttt{object}) turned out to be harder to handle than expected.  Here are the main problems we had:
\begin{itemize}
\item Because of \href{https://en.wikipedia.org/wiki/Object\_slicing}{object slicing}, we cannot create a vector of objects since this would call the copy constructor of the base class, and not of the derived classes. Our solution is to use a vector of pointers and to copy each object individually.  Because of this we have to manage three separate entities: the host vector of host pointers, the host array of device pointers, and the device array of device pointers.
\item Instances of derived classes can be bigger than their base class, which means we have to \texttt{dynamic\_cast} them individually to check their type and allocate the necessary space.
\item Most importantly, the pointer to the vtable is invalidated when transferred to the device since it is a \emph{host} pointer.  As-is, the objects are useless after the transfer.  Our solution is to check the objects individually again, and reinstantiate them from the kernel.  This is made harder by the fact that \texttt{dynamic\_cast} is not allowed from the device, preventing us from checking the dynamic types of the objects and instantiating them accordingly.
\end{itemize}
Of course, all these problems apply to the \texttt{light} class hierarchy as well.
\subsection{Thrust library}
To easily transfer vectors of objects/lights from host to device, we wanted to use the thrust library.  But we found out that thrust's \texttt{device\_vector}s are not meant to be used \emph{from the device}: none of their member functions have the \texttt{\_\_device\_\_} attribute except for the constructor.  Instead, they are meant to be manipulated from the host using the set of provided functions such as \texttt{thrust::transform}, \texttt{thrust::reduce} or \texttt{thrust::for\_each}.  Although we probably could have rewritten our raytracer using only these primitives, this would have defeated the purpose of the assignment.
